...
const AddressFormUI = (props) => {
        ...
        if (status === 'OK' && results && results.length > 0) {
          ...
          const arrayList = isEditing
            ? addressesList.filter(
                (address) => address.id !== addressState.address?.id
              ) || []
            : addressesList || []
          ...
          const isAddressAlreadyExist =
            arrayList
              .map((address) => checkAddress(address, addressToCompare))
  ...
  const setMapErrors = (errKey) => {
    setAlertState({
      ...
      content: !(errKey === 'ERROR_MAX_LIMIT_LOCATION')
        ...
        : `${[t(errKey, mapErrors[errKey])]} ${maxLimitLocation} ${[
            t('METTERS', 'meters')
          ]}`
    })
  }
  ...
  useEffect(() => {
    ...
    if (!isEditing) {
      inputNames.forEach((field) =>
        formMethods.setValue(field.name, formState?.changes[field.name] || '')
      )
      formState?.changes?.address &&
    ...
    inputNames.forEach((field) => {
      formMethods.register(field.name, {
        required: isRequiredField(field.name)
          ? t(
              `VALIDATION_ERROR_${field.name}_REQUIRED`,
              `The field ${field.code} is required`
            )
          : null
      })
      formMethods.setValue(
        field.name,
        formState.changes?.[field.name] ??
          addressState.address?.[field.name] ??
          ''
      )
    })
  }, [formMethods])
      ...
      {!configState.loading && !addressState.loading && (
        ...
        >
          {props.beforeMidElements?.map((BeforeMidElements, i) => (
            <React.Fragment key={i}>{BeforeMidElements}</React.Fragment>
          ))}
              ...
              (field.name === 'address' ? (
                <React.Fragment key={field.name}>
                  <AddressWrap className='google-control'>
                    <WrapAddressInput>
                      ...
                      <GoogleAutocompleteInput
                        ...
                        countryCode={
                          configState?.configs?.country_autocomplete?.value ||
                          '*'
                        }
                      />
                    </WrapAddressInput>
                  ...
                  {(addressState?.address?.location ||
                    formState?.changes?.location) &&
                    !toggleMap && (
                      <ShowMap onClick={() => setToggleMap(!toggleMap)}>
          ...
          {props.afterMidElements?.map((MidElement, i) => (
            <React.Fragment key={i}>{MidElement}</React.Fragment>
          ))}
