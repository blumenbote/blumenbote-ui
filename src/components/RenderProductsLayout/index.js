...
import {
  ...
  EmptyBtnWrapper,
  WrapperSearch,
  WrapperSearchAndBusinessCategories,
...
export const RenderProductsLayoutUI = (props) => {
  ...
  const isUseParentCategory =
    configs?.use_parent_category?.value === 'true' ||
    ...
    return (
      <FlexTabs>
        <Tabs variant='primary'>
          <Tab
            onClick={() => setCurrentCategory('flowers')}
            active={currentCategory === 'flowers'}
          >
            {t('PRODUCT_FLOWERS', 'Flowers')}
          </Tab>
          <Tab
            onClick={() => setCurrentCategory('generalInfo')}
          ...
          {business.reviews?.reviews && (
            <Tab
              onClick={() => setCurrentCategory('reviews')}
          ...
          {business?.offers?.length > 0 && (
            <Tab
              onClick={() => setCurrentCategory('offers')}
  ...
  const CategoryInners = () => {
    ...
    const isBusinessLayoutOne = businessLayout.layoutOne
    return (
      <>
                ...
                {(categoryState.products.length !== 0 || searchValue) && (
                  <WrapperSearch>
                    <SearchBar
                      ...
                      placeholder={t(
                        'SEARCH_PRODUCTS',
                        theme?.defaultLanguages?.SEARCH_PRODUCTS ||
                          'Search Products'
                      )}
                      lazyLoad={
                    ...
                    <Select
                      ...
                      onChange={(val) =>
                        handleChangeSortBy && handleChangeSortBy(val)
                      }
                    />
                  </WrapperSearch>
                )}
        ...
        {currentCategory === 'generalInfo' && (
          <>
            {business.about && (
              <>
                <SectionTitle>
                  {t('BUSINESS_ABOUT', 'Business short description')}
                </SectionTitle>
            ...
            {businessSchedule?.length > 0 && (
              <>
                ...
                <DeliveryInfo>
                  <SectionTitle>
                    {t(
                      'PRODUCT_DELIVERY_AND_PICKUP',
                      'Delivery and pickup time'
                    )}
                  </SectionTitle>
                  <DeliveryContent>
                    <h5>
            ...
            {businessVideos?.length > 0 && (
              <BusinessMediaContent>
                <SectionTitle>
                  {t('BUSINESS_VIDEOS', 'Business Videos')}
                </SectionTitle>
                <div>
    ...
    <>
      ...
      >
        <BusinessCartContent isModal>
          <Title style={{ textAlign: 'center', marginTop: '5px' }}>
